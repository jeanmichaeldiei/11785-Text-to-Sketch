    ...
    for i in range(len(data)):
        ...
        if len(threads) == 2:
            for t in threads:
                t.join()
            threads = []
        #apply_fn(results, i, data[i], kwargs)
...
def imresize(x, size=[100, 100], interp='bilinear', mode=None):
    ...
    if x.shape[-1] == 1:
        # greyscale
        x = skimage.transform.resize(x[:,:,0], size)#, interp=interp, mode=mode)
        return x[:, :, np.newaxis]
    elif x.shape[-1] == 3:
        # rgb, bgr ..
        return skimage.transform.resize(x, size)#, interp=interp, mode=mode)
    else:
